    ...
    private func bottomSheetContent(for route: BottomSheetRoute) -> some View {
            ...
            AddPreferencesForMemberSheet(
                ...
                yesPressed: {
                    Task { @MainActor in
                        ...
                        await foodNotesStore.clearOwnerState(for: memberId)
        ...
        case .meetYourAvatar:
            ...
            MeetYourAvatar(
                ...
                let newMemberInfo = await handleAssignAvatar(
                    memojiStore: memojiStore,
                    familyStore: familyStore,
                    webService: webService
                )

                // If a new member was created AND we're on home screen, go to invite screen
                if let (memberId, name) = newMemberInfo,
                   case .home = coordinator.currentCanvasRoute {
                    coordinator.navigateInBottomSheet(.wouldYouLikeToInvite(memberId: memberId, name: name))
                ...
                if let previousRoute = memojiStore.previousRouteForGenerateAvatar {
                    ...
                    if case .meetYourProfile(let memberId) = previousRoute {
                        ...
                        if case .mainCanvas = coordinator.currentCanvasRoute {
                            ...
                            // Not in onboarding, go back to invite screen if new member was created
                            if let (memberId, name) = newMemberInfo {
                                coordinator.navigateInBottomSheet(.wouldYouLikeToInvite(memberId: memberId, name: name))
                            } else {
        ...
        case .onboardingStep(let stepId):
            ...
            if let step = store.step(for: stepId) {
                let preferencesBinding = Binding(
...
private func handleAssignAvatar(
    ...
    familyStore: FamilyStore,
    webService: WebService
) async -> (memberId: UUID, name: String)? {
    ...
    if targetMemberId == nil,
       let name = displayName,
        ...
        if currentFamily != nil, let routeMemberId = memberIdFromRoute {
